...
export default function CapabilitiesScreen() {
  ...
  async function refresh() {
    try { setError(null); setCapabilities(await listMyCapabilities()); }
    catch (err) { setError(err instanceof Error ? err.message : "Unable to load capabilities"); }
  }
  ...
  async function addCapability() {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : "Unable to save capability"); }
  ...
  return (
    <div className="pb-24" style={{ color: CSP_TEXT_PRIMARY }}>
      ...
      <header style={{ marginBottom: CSP_SECTION_GAP }}>
        <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs"><Sparkles size={14} style={{ color: CSP_PRIMARY_BUTTON }} /><span style={{ color: CSP_TEXT_SECONDARY }}>What you can create</span></div>
        <h1 className="text-2xl font-semibold">Capabilities</h1>
        <p className="mt-2 text-sm leading-6" style={{ color: CSP_TEXT_SECONDARY }}>Cleaning is one capability, not your permanent identity. Track skills and strengths that can create value for households, other CSPs, businesses, and the network.</p>
      </header>
      ...
      <section style={{ marginBottom: CSP_SECTION_GAP }}>
        <div className="rounded-2xl border" style={{ backgroundColor: CSP_SURFACE, borderColor: "rgba(248,250,252,.08)", padding: CSP_CARD_PADDING }}>
          ...
          <p className="mt-1 text-xs leading-5" style={{ color: CSP_TEXT_SECONDARY }}>Self-declared capabilities are yours to add. Cleanr-verified capabilities remain separate so provenance stays clear.</p>
      ...
      <section>
        ...
        {capabilities.length === 0 ? (
          <div className="rounded-2xl border" style={{ backgroundColor: CSP_SURFACE, borderColor: "rgba(248,250,252,.08)", padding: CSP_CARD_PADDING }}>
            ...
            <p className="mt-1 text-xs leading-5" style={{ color: CSP_TEXT_SECONDARY }}>As Cleanr learns what you do well, this can include service expertise, mentoring, leadership, business skills, and other strengths.</p>
          </div>
        ) : <div className="space-y-3">{capabilities.map((capability) => <div key={capability.id} className="rounded-2xl border" style={{ backgroundColor: CSP_SURFACE, borderColor: "rgba(248,250,252,.08)", padding: CSP_CARD_PADDING }}><div className="flex items-start gap-3"><BadgeCheck size={19} style={{ color: capability.status === "verified" ? CSP_PRIMARY_BUTTON : CSP_TEXT_SECONDARY, marginTop: 2 }} /><div><p className="text-sm font-medium">{capability.label}</p><p className="mt-1 text-xs" style={{ color: CSP_TEXT_SECONDARY }}>{sourceLabel(capability.source)} · {capability.status}</p></div></div></div>)}</div>}
      </section>
    </div>
  );
}
