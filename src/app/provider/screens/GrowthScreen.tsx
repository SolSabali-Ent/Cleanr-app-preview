...
export default function GrowthScreen() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!active) return;
        setError(err instanceof Error ? err.message : "Unable to load your North Star");
      } finally {
  ...
  const navItems = [
    { label: "Milestones", description: "Break your direction into steps", count: milestoneCount, icon: Sparkles, route: CSP_GROWTH_ROUTES.milestones },
    { label: "Skills & strengths", description: "Record what you can do", count: capabilityCount, icon: Lightbulb, route: CSP_GROWTH_ROUTES.capabilities },
    { label: "Opportunities", description: "See paths aligned with where you're going", count: matchedOpportunityCount, icon: Compass, route: CSP_GROWTH_ROUTES.opportunities },
    { label: "Network", description: "Households, coverage, mentors, and connections", count: null, icon: Users, route: CSP_GROWTH_ROUTES.network },
    { label: "Impact", description: "See what your work has helped create for others", count: contributionCount, icon: Network, route: CSP_GROWTH_ROUTES.contributions },
  ];
    ...
    <div className="pb-24" style={{ color: CSP_TEXT_PRIMARY }}>
      <header style={{ marginBottom: CSP_SECTION_GAP }}>
        ...
        <p className="mt-1 text-sm" style={{ color: CSP_TEXT_SECONDARY }}>
          What are you building toward, and what can the Cleanr network help make possible?
        </p>
      </header>
        ...
        <div className="rounded-2xl border" style={{ backgroundColor: CSP_SURFACE, borderColor: "rgba(248,250,252,.08)", padding: CSP_CARD_PADDING }}>
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-xs font-medium" style={{ color: CSP_TEXT_SECONDARY }}>My direction</p>
              {northStar && !editing ? (
                <>
                  <p className="mt-1 text-lg font-semibold leading-6">{northStar.goal}</p>
                  {northStar.goal !== categoryLabel(northStar.category) ? <p className="mt-1 text-xs" style={{ color: CSP_TEXT_SECONDARY }}>{categoryLabel(northStar.category)}</p> : null}
                </>
              ) : !editing ? <p className="mt-1 text-sm font-medium">Choose what you&apos;re building toward.</p> : null}
            </div>
            {northStar && !editing ? <button type="button" onClick={() => setEditing(true)} className="shrink-0 text-xs font-semibold" style={{ color: CSP_PRIMARY_BUTTON }}>Edit</button> : null}
          </div>

          {loading ? <p className="mt-4 text-sm" style={{ color: CSP_TEXT_SECONDARY }}>Loading…</p> : editing || (!northStar && !isOfflinePreviewMode) ? (
            <div className="mt-4 space-y-3 border-t border-white/10 pt-4">
              ...
              <label className="block">
                <span className="mb-1 block text-xs" style={{ color: CSP_TEXT_SECONDARY }}>{category === "other" ? "Describe it" : "Make it yours (optional)"}</span>
                <textarea value={goal} onChange={(event) => setGoal(event.target.value)} rows={2} maxLength={500} placeholder={category === "other" ? "What are you building toward?" : "Add your own wording if you want"} className="w-full resize-none rounded-xl border border-white/10 bg-white/5 px-3 py-3 text-sm outline-none" />
              </label>
              <button type="button" disabled={!canSave} onClick={() => void handleSaveNorthStar()} className="flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold text-white disabled:opacity-50" style={{ backgroundColor: CSP_PRIMARY_BUTTON }}>{saving ? "Saving…" : northStar ? "Save changes" : "Set North Star"}</button>
              {northStar ? <button type="button" onClick={() => { setEditing(false); setCategory(northStar.category); setGoal(northStar.goal); }} className="w-full py-1 text-xs" style={{ color: CSP_TEXT_SECONDARY }}>Cancel</button> : null}
            </div>
          ) : !northStar && isOfflinePreviewMode ? <button type="button" disabled className="mt-4 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold opacity-70">Set North Star</button> : null}
        </div>
      </section>
      ...
      {showProgress ? (
        <section style={{ marginBottom: CSP_SECTION_GAP }}>
          <div className="flex items-center justify-between gap-3 border-y border-white/10 py-3 text-center">
            <div className="flex-1"><p className="text-base font-semibold">{servicePractice.repeatHouseholdsCount}</p><p className="mt-0.5 text-[10px]" style={{ color: CSP_TEXT_SECONDARY }}>Repeat homes</p></div>
            <div className="h-8 w-px bg-white/10" />
            <div className="flex-1"><p className="text-base font-semibold">{milestoneCount}</p><p className="mt-0.5 text-[10px]" style={{ color: CSP_TEXT_SECONDARY }}>Milestones</p></div>
            <div className="h-8 w-px bg-white/10" />
            <div className="flex-1"><p className="text-base font-semibold">{matchedOpportunityCount}</p><p className="mt-0.5 text-[10px]" style={{ color: CSP_TEXT_SECONDARY }}>Matches</p></div>
          </div>
        </section>
      ) : null}

      <section>
        <h2 className="mb-2 text-sm font-medium" style={{ color: CSP_TEXT_SECONDARY }}>Build toward it</h2>
        <div className="overflow-hidden border-y border-white/10">
          {navItems.map((item, index) => {
            const Icon = item.icon;
            return <button key={item.label} type="button" onClick={() => navigate(item.route)} className={`flex w-full items-center gap-3 py-4 text-left ${index > 0 ? "border-t border-white/10" : ""}`}>
              <Icon size={18} className="shrink-0" style={{ color: CSP_PRIMARY_BUTTON }} />
              <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="text-sm font-medium">{item.label}</p>{item.count !== null && item.count > 0 && !isOfflinePreviewMode ? <span className="text-[10px]" style={{ color: CSP_TEXT_SECONDARY }}>{item.count}</span> : null}</div><p className="mt-0.5 text-xs" style={{ color: CSP_TEXT_SECONDARY }}>{item.description}</p></div>
