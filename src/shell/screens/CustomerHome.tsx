...
import { CalendarDays, CalendarPlus, ChevronRight, MessageCircle } from "lucide-react";
...
function bookingState(booking: Booking, providerName: string | null): HomeBookingState {
  ...
  if (booking.status === "completed_by_provider") {
    return {
      eyebrow: "Cleaning finished",
      headline: "Your home is ready for you.",
      note: `${name} has finished. Take a quick look when you're ready.`,
      actionLabel: "Review cleaning",
    };
  }

  if (booking.status === "in_progress") {
    return {
      eyebrow: "Cleaning in progress",
      headline: `${name} is taking care of your home.`,
      note: "We'll let you know when the cleaning is finished.",
      actionLabel: "View cleaning",
    };
  }

  if (booking.provider_arrived_at) {
    return {
      eyebrow: "Your cleaner has arrived",
      headline: `${name} is at your home.`,
      note: "Everything for this visit is in one place.",
      actionLabel: "View cleaning",
    };
  }
  ...
  if (booking.provider_en_route_at) {
    return {
      ...
      note: "We'll keep the important updates right here.",
      actionLabel: "View cleaning",
    };
  }

  return {
    eyebrow: "Next cleaning",
    headline: "You're all set.",
    note: providerName ? `${providerName} is scheduled for your next visit.` : "Your next visit is scheduled.",
    actionLabel: "View cleaning",
  };
}
...
export function CustomerHome() {
  ...
  useEffect(() => {
    ...
    async function loadHome() {
      ...
      const row =
        ...
        rows.find((item) => item.status === "accepted") ??
        null;
  ...
  const state = useMemo(
    () => (upcoming ? bookingState(upcoming, providerName) : null),
    [upcoming, providerName]
  );
  const canMessage = Boolean(
    upcoming?.provider_id && isProviderCustomerMessagingOpen(upcoming?.status)
  );
    ...
    <div className="text-[#0B1220] pb-4">
      <header className="mb-6 section">
        <p className="text-xs font-medium text-[#166534]">
          {customerName ? `Hi, ${customerName}` : "Welcome back"}
        </p>
        <h1 className="home-hero-title mt-1">
          {loading || upcoming ? "We've got you." : "Ready when you are."}
        </h1>
        <p className="home-hero-sub mt-2 max-w-[34rem]">
          {loading || upcoming
            ? "Your cleaning and the updates that matter are right here."
            : "Book when you need a hand. We'll keep everything organized from there."}
        </p>
      </header>
      ...
      <section className="mb-5 section">
        {loading ? (
          <div className="provider-card p-5">
            <p className="text-sm text-[#667085]">Loading your home...</p>
          </div>
        ) : upcoming && state ? (
          <div className="next-cleaning-card p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="shrink-0 rounded-full border border-[#D0D5DD] bg-white p-2 text-[#667085]"
            ...
            <div className="mt-5 rounded-2xl bg-white/70 p-4">
              ...
              {providerName ? (
                <p className="mt-2 text-sm text-[#667085]">With {providerName}</p>
              ) : null}
            </div>
            ...
            <div className={`mt-4 grid gap-2 ${canMessage ? "grid-cols-2" : "grid-cols-1"}`}>
              ...
              {canMessage ? (
                <Button
                  onClick={() => navigate(`/app/bookings/${upcoming.id}/message`)}
                  variant="secondary"
                  size="md"
                  fullWidth
                >
                  {providerName ? `Message ${providerName}` : "Message cleaner"}
                </Button>
              ) : null}
            </div>
          </div>
          ...
          <div className="next-cleaning-card p-5">
            ...
            <Button onClick={() => navigate("/book")} variant="primaryGreen" size="lg" fullWidth className="mt-5">
              Book a cleaning
            </Button>
          </div>
        )}
      ...
      <section className="section">
        ...
        <div className="overflow-hidden rounded-2xl border border-[#E4E7EC] bg-white">
          <button
            ...
            className="flex min-h-[72px] w-full items-center gap-3 px-4 py-4 text-left"
          >
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#8DCC64]/15 text-[#166534]">
              <CalendarPlus className="h-5 w-5" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold">Book another cleaning</p>
              <p className="mt-0.5 text-xs text-[#667085]">Pick a day that works for you.</p>
            </div>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-xs text-[#667085]">Past and upcoming cleanings.</p>
            </div>
            <ChevronRight className="h-4 w-4 shrink-0 text-[#98A2B3]" />
          </button>
        </div>
        ...
        {upcoming?.provider_id ? (
          <button
            ...
            className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl py-3 text-sm font-medium text-[#166534]"
          >
            <MessageCircle className="h-4 w-4" />
            {providerName ? `Your cleaner: ${providerName}` : "Your cleaner"}
          </button>
        ) : null}
      </section>
    </div>
