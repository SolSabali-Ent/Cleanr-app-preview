...
function mapOpportunityPreferences(row: OpportunityPreferenceRow): OpportunityFitPreferences {
  return {
    personId: row.person_id,
...
export async function setMyOpportunityFitPreferences(input: {
...
}): Promise<OpportunityFitPreferences> {
  ...
  const { data, error } = await supabase.rpc("set_my_opportunity_preferences", {
    ...
    p_travel_radius_miles: input.travelRadiusMiles ?? null,
    p_fit_notes: input.fitNotes?.trim() || null,
  });
  if (error) throw error;
...
export async function listMyOpportunityMatches(): Promise<OpportunityMatch[]> {
  ...
  return rows.flatMap((row) => {
    ...
    return [{
      ...
      capabilityAlignment: row.capability_alignment,
      interestAlignment: row.interest_alignment,
