 ...
 * hosted schema does not yet contain newer Transformation tables/RPCs/columns. Reads should fail
 * closed to empty durable state instead of making the whole screen unusable. Writes still stay
 * disabled and surface a clear feature-not-active message.
 */
export function isSupabaseFeatureUnavailable(error: SupabaseLikeError | null | undefined): boolean {
  ...
  return code === "42P01" // undefined_table
    || code === "42703" // undefined_column during a partially applied schema
